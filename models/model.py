...
class GatedCNNBlock(nn.Module):
    r""" Our implementation of Gated CNN Block:
    Args: 
        conv_ratio: control the number of channels to conduct depthwise convolution.
            Conduct convolution on partial channels can improve paraitcal efficiency.
            The idea of partical channels is from Xception and 
            also used by InceptionNeXt and FasterNet 
    """
...
class GHCS(nn.Module):
    ...
    def forward(self, x):
		...
		@register_model
def GHCS(pretrained=False, **kwargs):
    model = GHCS(
        depths=[3, 3, 9, 3],
        dims=[48, 96, 192, 288],
        **kwargs)
    model.default_cfg = default_cfgs['GHCS']
    if pretrained:
	...
	@register_model
defGHCS_base(pretrained=False, **kwargs):
    model = GHCS(
        depths=[3, 4, 27, 3],
